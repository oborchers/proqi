...
use super::{
    ...
    schema::{
        ...
        MIGRATION_20, MIGRATION_21, MIGRATION_22,
    },
    support::{
...
pub(super) fn migrate(
    ...
    let transaction = connection
        ...
        .map_err(map_sql_error)?;
    apply_migrations(&transaction, found)?;
    transaction
        .execute(
            "UPDATE schema_meta
...
fn apply_migrations(connection: &Connection, found: u32) -> Result<(), StoreError> {
    if found == 0 {
        return connection.execute_batch(MIGRATION_1).map_err(map_sql_error);
    }
    let forward = [
        MIGRATION_2,
        MIGRATION_3,
