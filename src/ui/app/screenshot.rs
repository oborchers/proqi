...
impl BoardApp {
    ...
    pub(crate) fn screenshot_started(&mut self) {
        self.screenshot_state = ScreenshotState::Listening;
        self.screenshot_takeover = None;
        self.set_info("Screenshot Inbox is listening");
    }

    pub(crate) fn screenshot_stopped(&mut self) {
        self.screenshot_state = ScreenshotState::Off;
        self.screenshot_takeover = None;
    ...
    pub(crate) fn screenshot_conflict(&mut self, owner: CaptureOwnerInfo) {
        self.screenshot_state = ScreenshotState::Off;
        self.screenshot_takeover = Some(owner);
        self.screenshot_takeover_selected = 0;
    ...
    pub(crate) fn queue_screenshot_candidates(
        ...
        if self.screenshot_save.is_none() && self.screenshot_candidates.is_empty() {
            self.screenshot_notice_count = 0;
        }
        self.screenshot_candidates.extend(candidates);
    }
    ...
    pub(crate) fn advance_screenshot_capture(
        ...
        if self.screenshot_save.is_some()
            || self.pending_edit.is_some()
            || !matches!(self.state.durability, DurabilityState::Durable { .. })
        {
            return Vec::new();
        }
        let Some(candidate) = self.screenshot_candidates.pop_front() else {
            return Vec::new();
        };
    ...
    pub(crate) fn complete_screenshot_capture(
        ...
        let Some(ScreenshotSave::InFlight(commit)) = self.screenshot_save.take() else {
        ...
        let advance_auto_ready = self.screenshot_auto_ready.is_some_and(|thought_id| {
        ...
        match result {
            Ok(outcome) => match apply_capture(&mut self.state, &commit, &outcome) {
                Ok(Some(thought_id)) => {
                    ...
                    self.screenshot_auto_ready = (make_ready
                        && !self.screenshot_candidates.is_empty())
                    .then_some(thought_id);
                    self.sync_editor_from_state();
                    self.screenshot_notice_count = self.screenshot_notice_count.saturating_add(1);
                    let count = self.screenshot_notice_count;
                ...
                Ok(None) => {
                    if self.screenshot_candidates.is_empty() {
                        self.screenshot_auto_ready = None;
                    }
                }
                Err(error) => {
                    self.screenshot_auto_ready = None;
            ...
            Err(error) => {
                self.screenshot_save = Some(ScreenshotSave::Ready(commit));
    ...
    pub(super) fn handle_screenshot_takeover_input(
        ...
        match input {
            ...
            UiInput::Key(UiKey::Move {
                ...
                ..
            }) => self.screenshot_takeover_selected = 1,
    ...
    pub(super) fn choose_screenshot_takeover(&mut self, ids: &mut impl IdGenerator) -> Vec<Effect> {
        if self.screenshot_takeover_selected == 0 {
            self.cancel_screenshot_takeover();
            return Vec::new();
        }
        let Some(owner) = self.screenshot_takeover.take() else {
            return Vec::new();
        };
        self.screenshot_state = ScreenshotState::Starting;
    ...
    pub(in crate::ui) fn screenshot_takeover_view(&self) -> Option<(Vec<String>, usize)> {
        self.screenshot_takeover.as_ref().map(|_| {
            (
                vec!["Cancel".to_owned(), "Take over".to_owned()],
                self.screenshot_takeover_selected,
            )
        })
    }
}
