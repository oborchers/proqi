use super::*;
use crate::domain::{Separator, SeparatorId, ThoughtName};
...
fn set_names(state: &mut AppState, named: &[bool]) {
    let thought_ids = state
        ...
        .collect::<Vec<_>>();
    assert_eq!(thought_ids.len(), named.len());
    for (index, (thought_id, named)) in thought_ids.into_iter().zip(named).enumerate() {
        let name = named.then(|| {
            ThoughtName::new(format!("name {index}")).expect("valid fixture thought name")
        });
        state
            .board
            .thought_mut(thought_id)
            .expect("fixture thought")
            .set_name(name);
    }
}
...
#[test]
fn name_and_boundary_matrix_has_one_owner_for_every_row() {
    for density in [
        crate::ui::settings::BoardDensity::Comfortable,
        crate::ui::settings::BoardDensity::Compact,
    ] {
