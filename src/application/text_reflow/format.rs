...
mod isolated;
mod projection;
pub(crate) use projection::position_changes;
#[cfg(test)]
mod tests;
...
pub(crate) struct ReflowedText {
    pub(crate) content: String,
    pub(crate) changes: TextChangeSet,
    pub(crate) isolated: Vec<(Range<usize>, Range<usize>)>,
}

#[derive(Clone, Debug, Eq, PartialEq, thiserror::Error)]
pub(crate) enum ReflowError {
...
pub(crate) fn reflow_text_isolated(
    content: &str,
    protected: &[Range<usize>],
...
fn unchanged(content: &str, isolated: &[Range<usize>]) -> ReflowedText {
    ReflowedText {
