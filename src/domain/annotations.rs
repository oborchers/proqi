...
use super::{ContentAnnotation, DomainError};
...
///
/// An annotation crossing the boundary is represented by one fragment on each
/// side with the same provenance. The right side is rebased to byte zero.
///
/// # Errors
///
...
pub fn partition_annotations(
    ...
    for annotation in annotations {
        push_fragment(
            ...
            annotation,
            annotation.start,
            annotation.end.min(at),
            0,
        );
        push_fragment(
            &mut right,
            annotation,
            annotation.start.max(at),
            annotation.end,
            at,
        );
    }
...
pub fn extract_annotations(
    ...
    for annotation in annotations {
        push_fragment(
            &mut extracted,
            annotation,
            annotation.start.max(range.start),
            annotation.end.min(range.end),
            range.start,
        );
        let retained = if annotation.end <= range.start {
            Some((annotation.start, annotation.end))
        } else if annotation.start >= range.end {
            Some((annotation.start - removed, annotation.end - removed))
        } else {
            match (annotation.start < range.start, annotation.end > range.end) {
                (true, true) => Some((annotation.start, annotation.end - removed)),
                (true, false) => Some((annotation.start, range.start)),
                (false, true) => Some((range.start, annotation.end - removed)),
                (false, false) => None,
            }
        };
        if let Some((start, end)) = retained
            && start < end
        {
...
fn push_fragment(
    target: &mut Vec<ContentAnnotation>,
    source: &ContentAnnotation,
    start: usize,
    end: usize,
    offset: usize,
) {
    if start < end {
        target.push(ContentAnnotation {
            start: start - offset,
            end: end - offset,
