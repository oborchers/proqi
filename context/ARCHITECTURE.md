...
prefix bytes, including tabs, and otherwise inserts configured spaces aligned to
the preceding parent content column. Outdent deletes only a recognized parent
suffix or a complete conservative indentation unit and never renumbers later
ordered markers. Outside recognized list context, Tab is exact space insertion
and BackTab is a no-op. Palette commands route through the same intentions for
modifier-independent keyboard and mouse access.
