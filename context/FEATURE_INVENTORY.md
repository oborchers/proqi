...
Status: current `v0.2.0` release candidate, reviewed 2026-08-30.
...
| Sentence deletion | Experimental | `Primary+Shift+U`, the Commands overlay, and a configured shifted Primary chord delete every Unicode sentence touched by the cursor or selection. `Primary+U` remains logical-line deletion. The documented UAX 29 profile preserves single prose newlines and hard blank-line boundaries without claiming linguistic certainty. | `docs/EXPERIMENTAL_SENTENCE_DELETION.md`, `tests/editor_contract/sentences.rs`, `tests/ui_board/sentence_deletion.rs`, `tests/pty/sentence_deletion.rs` |
