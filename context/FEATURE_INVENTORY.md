...
| Multi-selection | Shipped on current main | `Space` selects thoughts. Copy, cut, delete, collapse, and one-call Herdr submission preserve board order and use one undoable structural operation. | `tests/ui_board/selection.rs`, `tests/ui_board/agent_selection.rs`, `tests/sqlite_store/bulk.rs` |
