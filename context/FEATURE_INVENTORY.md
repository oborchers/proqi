...
| Explicit paste reflow | Shipped | Exact paste remains the default. `Primary+Shift+V`, the collision-safe Board `p` and `P` fallbacks, and Commands clean terminal-wrapped prose while protecting structured and annotated content as one persistent undo unit. | `src/ui/paste_reflow.rs`, `tests/ui_board/paste_reflow.rs`, `tests/pty/key_inspector.rs` |
