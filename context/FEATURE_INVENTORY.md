...
| Reordering | Implemented | Mouse drag, `Meta+Shift+Up` / `Meta+Shift+Down`, and equivalent `Meta+K` / `Meta+J` chords move one thought and wrap at board edges. Selected-block reorder is intentionally unavailable. | `tests/ui_board/navigation.rs`, `tests/ui_board.rs` |
