...
| Reordering | Implemented | Mouse drag, `Primary+Shift+Up` / `Primary+Shift+Down`, and equivalent `Primary+K` / `Primary+J` chords move one thought and wrap at board edges. Selected-block reorder is intentionally unavailable. | `tests/ui_board/navigation.rs`, `tests/ui_board.rs` |
