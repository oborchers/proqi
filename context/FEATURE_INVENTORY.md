...
| First-run practice board | Shipped | The first eligible fresh interactive session in a pristine data store receives six ordinary persisted thoughts once. Reviewed shortcut literals use semantic emphasis and spell Primary as Cmd on macOS or Ctrl elsewhere. Existing practice boards are never rewritten. Noninteractive commands, resume, continue, browsing, and later fresh sessions do not seed or consume the board. | `src/application/onboarding.rs`, `tests/sqlite_store/onboarding.rs`, `tests/pty/onboarding.rs`, `tests/ui_board/first_run.rs` |
...
| Thought creation | Shipped | `n`, the keyboard-focusable insertion row, its mouse target, board paste, and two consecutive downward navigation commands on the insertion row create thoughts. On macOS, exact logical Control plus `n` inserts below and Control plus Shift plus `n` inserts above, with uppercase-without-Shift compatibility. Portable exact Alt plus Up or Down, with configured `k` and `j` aliases, retains directional insertion. An explicitly created blank is durable. | `tests/ui_board/blank.rs`, `tests/ui_board/insertion_navigation.rs`, `tests/ui_board/terminal_safe_navigation.rs`, `tests/pty/terminal_safe_navigation.rs`, `tests/ui_mouse_actions.rs` |
| Persistent visual separators | Implemented | Commands inserts an unnamed, payload-free separator below the focused item. Separators persist, focus, select, reorder, drag, duplicate, delete, undo, and redo as ordinary structural Board items. They suppress adjacent automatic dividers and never enter copied or submitted text. | `tests/domain_reducer/separators.rs`, `tests/sqlite_store/separators.rs`, `tests/ui_board/separators.rs`, `tests/ui_board/separator_snapshots.rs` |
| Board boundary navigation | Shipped | Exact logical Control plus Up or Down, with configured `k` and `j` aliases, clamps focus to the first or last live thought. macOS Control+Shift extends the existing range anchor to that boundary. Portable Control+Shift remains thought reordering. | `tests/ui_board/terminal_safe_navigation.rs`, `tests/pty/terminal_safe_navigation.rs`, `tests/pty/key_inspector.rs` |
...
| Selection and cursor | Shipped | Keyboard selection includes grapheme movement and fold-aware extension to current wrapped visual-row edges. Single-click placement, character dragging, double-click word selection, triple-click logical-line selection, granular dragging, Shift-click extension, wrapped cursor placement, five-visual-row jumps, exact thought-boundary movement, trailing newlines, and a terminal-owned blinking cursor are supported. | `tests/editor_contract.rs`, `tests/ui_board/visual_row_selection.rs`, `tests/ui_board/fast_navigation.rs`, `tests/ui_board/pointer_selection.rs`, `tests/pty/key_inspector.rs`, `tests/pty/visual_row_selection.rs` |
...
| Reordering | Implemented | Mouse drag, `Primary+Shift+Up` / `Primary+Shift+Down`, equivalent `Primary+K` / `Primary+J`, and macOS `Option+Shift+Up` / `Option+Shift+Down` chords move one thought and wrap at board edges. Selected-block reorder is intentionally unavailable. | `tests/ui_board/navigation.rs`, `tests/pty/reorder.rs`, `tests/ui_board.rs` |
...
| Thought transformations | Implemented | `Primary+T` splits at the exact editor cursor or extracts the exact selection. The remappable board `t` merges a contiguous selection with the configured exact separator. Each transformation is one restart-safe durable operation, and the command palette provides keyboard and mouse discovery. | `tests/domain_reducer/transformations.rs`, `tests/sqlite_store/transformations.rs`, `tests/ui_board/transformations.rs`, `tests/pty/transformations.rs` |
...
| Fuzzy invocation lookup | Shipped | `$`, `/`, and supported `@` forms use deterministic token-first fuzzy ranking with exact sigil compatibility, Unicode normalization, stable discovery-order ties, complete result retention, and keyboard and mouse selection beyond the visible viewport. | `src/ui/app/invocation/matcher.rs`, `src/ui/app/invocation/ranking_tests.rs`, `tests/pty/invocation.rs` |
| Shared harness built-ins | Conditional on a verified adjacent Codex or Claude Code target | The checked-in 19-token catalog completes and highlights only at byte zero. Selection inserts text without executing it. Only `/plan` and `/goal` keep their established later-thought stripping policy. | `src/application/prompt.rs`, `src/ui/app/invocation/builtins.rs`, `src/ui/app/invocation/builtins_tests.rs`, `tests/ui_board/agent_selection.rs` |
...
| Clean up thought spacing | Implemented | Plain Board `f`, Edit `Control+Shift+F`, and Commands clean one complete durable thought using the shared paste classifier, including collapsed large-paste content. Authored lines and one paragraph gap remain; retained folds recount and preserve presentation; protected annotations remain exact. Changed results are one history step and no-ops create no entry. | `src/ui/app/reflow.rs`, `tests/ui_board/reflow_in_place.rs`, `tests/sqlite_store/reflow_in_place.rs`, `tests/pty/reflow_in_place.rs` |
| Explicit paste cleanup | Shipped | Exact paste remains the default through `Primary+V` or Board `p`; `Primary+Shift+V`, paired Board `P`, and Commands collapse excess spacing while preserving authored lines, structured content, and annotations as one persistent undo unit. | `src/ui/paste_reflow.rs`, `tests/ui_board/paste_reflow.rs`, `tests/pty/key_inspector.rs` |
...
| Herdr discovery | Conditional | In a managed Herdr pane, Proqi discovers and independently verifies adjacent coding agents in all four directions. Protocols 19, 20, and 21 are qualified, while protocol 22 is provisionally accepted only under the same schema 1 contract. A scheduled sentinel opens one deduplicated qualification issue when the latest official release is provisional or incompatible. Ready empty Codex, Kilo, and OpenCode instances may be provisional; a receipt that precedes a session hook triggers immediate rediscovery without resending. | `src/adapters/herdr`, `tests/herdr_executable.rs`, `.github/workflows/herdr-compatibility.yml` |
| Submit and keep | Conditional | A verified adjacent target receives one exact prompt through Herdr's semantic command, while every selected source thought remains. Concurrent senders can expose the supported Herdr protocol prompt-boundary limitation. | `tests/ui_board/agent.rs`, `tests/ui_board/agent_selection.rs`, `src/adapters/herdr/submission.rs` |
| Submit | Conditional | The default semantic submission removes all unchanged selected sources as one operation only after the matching accepted receipt. Failure and ambiguity preserve them. | `tests/ui_board/agent.rs`, `tests/ui_board/agent_selection.rs`, `src/ui/app/agent.rs` |
| Submit complete board | Conditional | Palette actions submit every live thought through one verified request using canonical ordered assembly, either keeping all sources or removing them together only after durable accepted delivery. Empty boards, failure, target change, and ambiguous outcomes preserve the board. | `tests/ui_board/submit_all.rs`, `tests/sqlite_store/submission.rs`, `src/ui/app/agent_delivery.rs` |
...
| Standalone coordinated update | Shipped | After explicit confirmation, verified standalone participants save, one process downloads the exact release-bound installer and checksum from GitHub Releases, verifies the installer before execution, installs the requested version into the existing user-owned prefix, and every participant verifies the replacement before resuming in place. Homebrew keeps its existing coordinated upgrade lane. Debian, Cargo, and source installs remain non-mutating. | `src/adapters/update/standalone.rs`, `src/application/update_coordination.rs`, `tests/pty/update_control.rs`, `README.md` |
...
| Debian package | Verified release contract | The immutable candidate builds `proqi_amd64.deb` and `proqi_arm64.deb` from their exact verified GNU Linux archive binaries, derives runtime dependencies, omits maintainer scripts, preserves user state on removal, and tests each package on Ubuntu 22.04, Ubuntu 24.04, and Debian bookworm for its architecture. They become public only with the authorized GitHub Release. There is no APT repository. | `xtask/src/debian.rs`, `xtask/src/debian_container.rs`, `.github/workflows/release-candidate.yml` |
...
| GNU/Linux compatibility | Candidate gate | The x86-64 and ARM64 GNU archives are built natively on Ubuntu 22.04, reject required symbols newer than `GLIBC_2.35`, and start from the final archives on Ubuntu 22.04, Debian bookworm, and Ubuntu 24.04 before promotion. Static x86-64 and ARM64 musl archives provide the verified fallback for musl systems and systems below the GNU libc floor. | `xtask/src/linux_compat.rs`, `.github/workflows/release-candidate.yml` |
...
isolated installed-product contract, Rust 1.88, the typed six-target release
