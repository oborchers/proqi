...
| Reflow thought in place | Implemented | Plain Board `f`, Edit `Primary+F`, and Commands reflow one complete durable thought using the paste classifier, including collapsed large-paste content. Retained folds recount and preserve presentation; protected annotations remain exact. Changed results are one history step and no-ops create no entry. | `src/ui/app/reflow.rs`, `tests/ui_board/reflow_in_place.rs`, `tests/sqlite_store/reflow_in_place.rs`, `tests/pty/reflow_in_place.rs` |
| Explicit paste reflow | Shipped | Exact paste remains the default through `Primary+V` or Board `p`; `Primary+Shift+V`, paired Board `P`, and Commands clean terminal-wrapped prose while protecting structured and annotated content as one persistent undo unit. | `src/ui/paste_reflow.rs`, `tests/ui_board/paste_reflow.rs`, `tests/pty/key_inspector.rs` |
