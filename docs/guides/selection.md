...
the target for editing, naming, single-item reordering, and any action used
without a selection.
...
## Reordering stays single-item

Keyboard and mouse reordering move only the focused item, not a selected block.
Use `Primary+Shift+Up` and `Primary+Shift+Down`, `Primary+K` and `Primary+J`, or
the drag handle. On macOS, `Option+Shift+Up` and `Option+Shift+Down` are
additional factory aliases.

Keyboard reordering wraps at Board edges. Dragging is positional and does not
wrap. Each move is saved and undoable.

## Undo a group action

A bulk delete, cut, duplicate, collapse, or accepted remove-after-delivery is
