//! Schema 21 session request receipt protocol stamp and refusal contracts.
...
const DOWNGRADE_TO_20: &str = r"
DELETE FROM migration_history WHERE version >= 21;
UPDATE schema_meta SET schema_version = 20, storage_protocol = 19;
";

// The published v0.12 shape, before the transfer journal and this stamp.
...
fn session_request_protocol_stamp_is_authorized_backed_up_and_preserves_receipts() {
    ...
    connection
        .execute_batch(DOWNGRADE_TO_20)
        .expect("schema 20 fixture");
    drop(connection);
...
fn a_legacy_schema_19_rename_receipt_decodes_and_replays_after_migration() {
    ...
    let mut store = fixture.open();
    store.quick_check().expect("migrated across 20 and 21");
    assert!(matches!(
        store.session_request(operation_id),
        Ok(Some(StoredSessionRequest::Administration(receipt)))
